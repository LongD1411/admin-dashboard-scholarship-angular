...
export class FieldOfStudyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      this.searchFieldOfStudy(this.keyword, this.currentPage, this.limit);
    });
  }
  searchFieldOfStudy(keyword: string, currentPage: number, limit: number) {
    this.fieldOfStudyService.getFieldOfStudies(keyword, currentPage, limit).subscribe({
      next: (response) => {
        this.fieldOfStudies = response.results;
  ...
  creatFieldOfStudyNavigate() {
    window.location.href = '/chuyen-nganh/chinh-sua';
  }
  viewFieldOfStudy(id: number) {
    window.location.href = `/chuyen-nganh/chinh-sua?id=${id}`;
  }

  deleteFieldOfStudy(id: number) {
    this.alert.showConfirm('Cảnh báo', 'Xác nhận xóa').then((result) => {
      if (result.isConfirmed) {
        this.fieldOfStudyService.deleteFieldOfStudy(id.toString()).subscribe({
          next: (response) => {
